...
def main(model, args):
    ...
    def process(raw_text):
        ...
        if args.with_id:
            ...
            result_list.append(txts[0])

    # os.makedirs(args.output_path, exist_ok=True)
    avg_time = generate_continually(process)
    import re

    results = []

    pattern_qa = re.compile(r'.*答案:(.*)')
    pattern_dialog = re.compile(r'.*B:(.*)')

    for each_answer in result_list:
        if args.test_mode == 'qa':
            pattern = pattern_qa
        else:
            pattern = pattern_dialog
        try:
    ...
    tgt_file = open(args.output_path, 'a')
    for each_line in results:
        tgt_file.write(each_line + '\n')
    tgt_file.write("avg_time:" + str(avg_time))
    tgt_file.close()
...
if __name__ == "__main__":
    ...
    py_parser.add_argument('--test_mode', type=str, default='qa')
